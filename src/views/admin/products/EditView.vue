<script setup>
    ...
    function save() {
        ...
        axios.post('/products/' + route.params.slug, formData, { headers: { 'Content-Type': 'multipart/form-data', 'Authorization' : 'Bearer ' + user.access_token } })
            .then(response => {
                loader.hide();
    ...
    onMounted(() => {
        ...
        axios.get('/products/' + route.params.slug, { headers: { 'Authorization' : 'Bearer ' + user.access_token } })
            .then(response => {
                header.value = {...response.data.data.header};

                details.value = { ...details.value, ...response.data.data.details };

                footer.value = {...response.data.data.footer};

                settings.value = {...settings.value, ...response.data.data.settings};
                ...
                <form @submit.prevent="save()" class="w-full flex flex-col justify-center items-center">
                    <TabGroup>
                        ...
                        <TabPanels class="mt-10 w-full">
                            ...
                            <TabPanel class="flex justify-center items-center" :unmount="false">
                                <div class="w-full max-w-2xl flex flex-col gap-6">
                                    <input v-model="settings.checkout.url" type="text" placeholder="URL do checkout" class="w-full rounded-sm">
                                </div>
                            </TabPanel>
                        </TabPanels>
                    </TabGroup>

                    <button class="px-6 py-3 rounded-md mt-6 bg-gradient-to-tr from-[#2DCE89] to-[#2DCECC] text-white shadow">Atualizar</button>
                </form>
            </main>
        </div>
    </div>
